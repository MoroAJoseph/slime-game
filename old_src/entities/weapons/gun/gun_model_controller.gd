...
@onready var mesh_visual_scene: PackedScene = preload(Constants.MESH_VISUAL_SCENE_PATH)
...
func _assemble_recursive(part_data: GunPartData, parent_node: Node, transform_offset_data: TransformData, socket_type: GunResource.GunPartSocket) -> void:
	...
	if mesh_visual_scene == null:
		mesh_visual_scene = load(Constants.MESH_VISUAL_SCENE_PATH)
	...
	# --- MATERIAL LOGIC ---
	var base_mat = load(Constants.WEAPON_MATERIAL_PATH)
